...
void ImuProcess::IMU_Initial(const MeasureGroup &meas, StatesGroup &state_inout, int &N) {
    ...
    for (const auto &imu: meas.imu) {
        const auto &imu_acc = imu->linear_acceleration; // 加速度
        const auto &gyr_acc = imu->angular_velocity; // 角速度
        cur_acc << imu_acc.x, imu_acc.y, imu_acc.z;
        cur_gyr << gyr_acc.x, gyr_acc.y, gyr_acc.z;

        mean_acc += (cur_acc - mean_acc) / N; // 平均?
...
// measuregroup:（struct） Lidar data and imu dates for the curent process
void ImuProcess::lic_state_propagate(const MeasureGroup &meas, StatesGroup &state_inout) {
    /*** add the imu of the last frame-tail to the of current frame-head ***/
    // 将上一帧尾的imu添加到当前帧头的imu
    auto v_imu = meas.imu;
    v_imu.push_front(last_imu_); // add at front(head)
